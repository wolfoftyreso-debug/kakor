...
describe("rate limiting", () => {
  it("släpper igenom upp till gränsen och blockerar sedan", () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(rateLimitMemory(key, { limit: 5, windowMs: 60_000 }).ok).toBe(true);
    }
    const blocked = rateLimitMemory(key, { limit: 5, windowMs: 60_000 });
  ...
  it("olika nycklar räknas separat", () => {
    const a = `a-${Date.now()}`;
    const b = `b-${Date.now()}`;
    for (let i = 0; i < 3; i++) rateLimitMemory(a, { limit: 3, windowMs: 60_000 });
    expect(rateLimitMemory(a, { limit: 3, windowMs: 60_000 }).ok).toBe(false);
    expect(rateLimitMemory(b, { limit: 3, windowMs: 60_000 }).ok).toBe(true);
  });
});
